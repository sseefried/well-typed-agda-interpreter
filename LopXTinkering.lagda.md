...
_y∈toΓΔ→y∈Δ_ : ∀ {x y t Δ} → (Δ′ : Δ [ x ::: t ]) → y ∈ toΓ Δ′ → y ∈ Δ
_y∈toΓΔ→y∈Δ_ {x} {y} {t} {Δ = ·}  (end .x .t) _ = {!!} -- this can't be proved! It's false.
_y∈toΓΔ→y∈Δ_ (shift y s (end x t))  H = H -- y ∈ x ::: s , · ---> y ∈ Δ (and x = y)
-- _y∈toΓΔ→y∈Δ_ {x} {y} (shift y′ _ (shift _ _ Δ)) H = ? 
-- ... | yes refl = Δ y∈toΓΔ→y∈Δ prf
...
-- _y∈toΓΔ→y∈Δ_ {x} {Δ = ·} (end x t) (H {.x})  = {!!} 
-- _y∈toΓΔ→y∈Δ_ {Δ = x ::: t , Δ} (shift .x .t Δ′) pf = Δ′ y∈toΓΔ→y∈Δ ?
-- _y∈toΓΔ→y∈Δ_ Δ@(shift y s _) (TH ⦃ prf = y∈Δ ⦄) = Δ y∈toΓΔ→y∈Δ {!!}

-- H  y∈toΓΔ→y∈Δ (shift y s Δ) = H
-- (TH ⦃ prf = y∈Δ ⦄) y∈toΓΔ→y∈Δ (shift y s Δ) = TH ⦃ prf = y∈Δ y∈toΓΔ→y∈Δ Δ ⦄ 
```
...
fromExt-Γ : ∀ {x y t s Δ} →  ⦃ Δ′ : Δ [ x ::: t ] ⦄ → ⦃ i : y ∈ toΓ Δ′ ⦄ → s ≡ !Γ (toΓ Δ′) [ i  ] → (s ≡ !Γ Δ [ Δ′ y∈toΓΔ→y∈Δ i ])
fromExt-Γ = {!!} 
...
This is what is available for a lop-x-var 

x  : Var   (not in scope)
t  : `Set   (not in scope)
tr : `Set   (not in scope)
Δ  : Γ   (not in scope)
Δ′ : Δ [ x ::: t ]   (not in scope, instance)
y  : Var
i  : y ∈ toΓ Δ′   (not in scope, instance)
eq : tr ≡ !Γ toΓ Δ′ [ i ]   (not in scope, instance)
...
--      → ⦃ eq : s ≡ !Γ Δ [ i ] ⦄
      → ⦃ eq : s ≡ !Γ (toΓ Δ′) [ i ] ⦄ 
      → ⦃ isVar :  e ≡ (`v y) ⦃ i = i ⦄ ⦃ eq = eq ⦄ ⦄
      → Δ ⊢ s
lop-x-var {x} {y} ⦃ Δ′ = Δ′ ⦄ ¬x≡y (`v _) ⦃ i = i ⦄ ⦃ eq = eq ⦄  = (`v y) ⦃ i = _y∈toΓΔ→y∈Δ_  Δ′  i ⦄ ⦃ eq = fromExt-Γ eq ⦄ 


test-arg :  z' ::: `Bool , (y' ::: `⊤ , (x' ::: `Bool , ·)) ⊢ `⊤
test-arg = `v y'

test-lop-x : z' ::: `Bool , (y' ::: `⊤ , ·) ⊢ `⊤
test-lop-x = lop-x-var (λ()) test-arg

```


Holy shit I cannot believe I managed to write `lop-x-var` succesfully! Look at it go! 


Okay, the next task is to work out how to write a `lop-x` function that doesn't just work on variables

```
data _nnfi_[_]⊢_∋_ : (x : Var) → (Δ : Γ) → (xt : `Set) → (Δᵉ : Δ [ x ::: xt ]) → (t : `Set) → (toΓ Δᵉ ⊢ t) → Set where
  nnfi-true   : ∀ {Δ x t} → ⦃ Δᵉ : Δ [ x ::: t ] ⦄ → _nnfi_[_]⊢_∋_ x Δ t Δᵉ `Bool `true
--  nnfi-false  : ∀ {x Δ} → x nfi Δ ⊢ `Bool ∋ `false
  nnfi-var    : ∀ {x y Δ t s} → ⦃ Δᵉ : Δ [ x ::: t ] ⦄ → ⦃ i : y ∈ Δ ⦄ → ⦃ eq : s ≡ !Γ Δ [ i ] ⦄ → ¬ (x ≡ y) → _nnfi_[_]⊢_∋_ x Δ t Δᵉ s ((`v y) ⦃ i = i y∈Δ→y∈toΓΔ Δᵉ ⦄ ⦃ eq = toExt-Γ eq ⦄)
--  nnfi-const  : ∀ {x Δ t c} → x nfi Δ ⊢ t ∋ `c c
--  nnfi-app    : ∀ {x Δ t s f arg} → x nfi Δ ⊢ t `⇨ s ∋ f → x nfi Δ ⊢ t ∋ arg → x nfi Δ ⊢ s ∋ f `₋ arg
--  nfi-lambda : ∀ {x Δ tx tr e} → x nfi Δ ⊢ tx `⇨ tr ∋ (`λ x `: tx ⇨ e)
--  nnfi-pair   : ∀ {x Δ t₁ t₂ e₁ e₂} → x nfi Δ ⊢ t₁ ∋ e₁ → x nfi Δ ⊢ t₂ ∋ e₂ → x nfi Δ ⊢ t₁ `× t₂ ∋ e₁ `, e₂
--  nnfi-fst    : ∀ {x Δ t₁ t₂ e } → x nfi Δ ⊢ t₁ `× t₂ ∋ e → x nfi Δ ⊢ t₁ ∋ `fst e
--  nnfi-snd    : ∀ {x Δ t₁ t₂ e } → x nfi Δ ⊢ t₁ `× t₂ ∋ e → x nfi Δ ⊢ t₂ ∋ `snd e
--  nnfi-tt     : ∀ {x Δ} → x nfi Δ ⊢ `⊤ ∋ `tt
```

```
lop-x : ∀ {x t tr Δ} → ⦃ Δ′ : Δ [ x ::: t ] ⦄ → (e : toΓ Δ′ ⊢ tr) → _nnfi_[_]⊢_∋_ x Δ t Δ′ tr e → Δ ⊢ t
--lop-x `true _ = `true
--lop-x `false _ = `false
lop-x ((`v y) ) (nnfi-var ⦃ _ ⦄ ⦃ i ⦄ ⦃ eq ⦄ ¬x≡y ) = ? 
--lop-x (`c c) _ = `c c
--lop-x (f `₋ a)(nfi-app n₁ n₂)  = lop-x f n₁ `₋ lop-x a n₂
--lop-x (`λ z `: u ⇨ e) _ = `λ z `: u ⇨ lop-x e {!!} 
--lop-x (e₁ `, e₂) (nfi-pair n₁ n₂) = lop-x e₁ n₁ `, lop-x e₂ n₂
--lop-x (`fst e) (nfi-fst n) = `fst (lop-x e n)
--lop-x (`snd e) (nfi-snd n)= `snd (lop-x e n)
--lop-x `tt _ = `tt
